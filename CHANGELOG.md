...
## 0.10.7
El usuario migró HAOS de una RPi5 a un i7 de 8 núcleos intentando resolver el patrón de cuelgues intermitentes de HA Core (v0.10.6), y seguía pasando — señal de que no era falta de CPU/E-disco bruta (un i7 de 8 núcleos no debería sudar con esto), sino contención de bloqueos.
...
## 0.10.6
**Causa raíz confirmada en producción** del patrón de cuelgues intermitentes de HA Core en RPi5: el usuario aisló el problema desactivando Battery Orchestrator primero (sin efecto, seguía pasando) y Climate Orchestrator después (se resolvió al instante) — con un log de monitorización externa (Pangolin) mostrando el patrón exacto: caídas de "unhealthy" cada ~8-10 minutos, justo el periodo de `forecast_refresh_minutes` (10 min por defecto).
...
## 0.10.4
Revisión a fondo de la lógica de cambio y asignación de modos dentro de Auto (heat_cool), a petición del usuario — reportó que la deshumidificación bajaba la temperatura por debajo de la consigna y disparaba gasto de más. Se encontraron y corrigieron varios problemas reales de eficiencia:
